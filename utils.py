...
def load_svhn(batch_size, test_batch_size, shuffle_train=True, shuffle_test=False):
    print('load svhn: batch size {0}, test_bs{1}'.format(batch_size, test_batch_size))
    train_dataloader = torch.utils.data.DataLoader(
        datasets.SVHN(
            "data/svhn",
            split='train',
...
def evaluate_losses(dataset, real_imgs, recon_imgs, discriminator, netQ, y_dim10):
    real_validity = discriminator(real_imgs)
    fake_validity = discriminator(recon_imgs)
    x_test_flat = recon_imgs.view(recon_imgs.shape[0], -1)
    if dataset == 'mnist':
        y2 = netQ(x_test_flat)
    else:
        y2 = netQ(recon_imgs)
    ...
    classi_loss = F.cross_entropy(y2, y_dim10).div(math.log(2))

    return distortion_loss, perception_loss, classi_loss, y2

#### Change
# def evaluate_losses(real_imgs, recon_imgs, discriminator, y, y_recon):
#     real_validity = discriminator(real_imgs)
#     fake_validity = discriminator(recon_imgs)
...
#     distortion_loss = F.mse_loss(real_imgs, recon_imgs)
#     #class label loss
#     class_loss = F.cross_entropy(y_recon, y).div(math.log(2))

#     return distortion_loss, perception_loss, class_loss
